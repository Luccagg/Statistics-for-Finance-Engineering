...
**Solution**:
>Just as with problem 1, we plot the returns without difficulty. The answer to the the possible correlation between the Microsoft and Merck returns is that are some correlation between the returns, but we have some positive outliers in Merck returns that are negative correlated with the Microsoft returns of the same period, like a point in coordinates ~ (1.0) for MRK and ~ (-0.15)
